...
<?php
  ...
  ?>
          <div class="">
          <a href="#" class="btn btn-danger rounded-pill text-left" style="width:100%"><b>INFORMASI TAMBAHAN WAKA KURIKULUM</b></a><br><br>
            <p align="left" <?=$hidden_mapel?> <?=$hidden_walas?> <?=$hidden_wakakur?> <?=$hidden_wakasis?>>
              <a class="btn btn-success font-weight-bold" href="<?=base_url('admin/tambah_info_wakakur')?>">TAMBAH INFO BARU</a>
            </p>
                ...
                <tbody>
                  ...
                  foreach ($info_tambahan as $data) : ?>
                    ...
                    <td scope="row"><?php echo $i; ?></td>
                     ...
                     <td><a  href="<?=base_url('admin/baca_infotambahan/').$data->id_info?>" class="btn btn-sm btn-success"><b>BACA INFO</u></a>
                        <a <?=$hidden_mapel?> <?=$hidden_wakakur?> <?=$hidden_wakasis?> <?=$hidden_walas?> href="<?=base_url('admin/edit_infotambahan/').$data->id_info?>" class="btn btn-sm btn-primary"><b>EDIT INFO</u></a>
                        <a <?=$hidden_mapel?> <?=$hidden_wakakur?> <?=$hidden_wakasis?> <?=$hidden_walas?> href="<?=base_url('admin/hapus_infotambahan/').$data->id_info?>" class="btn btn-sm btn-danger"><b>HAPUS INFO</u></a>
                    </td>
                    </tr>
                    <?php $i++; ?>
                  <?php endforeach; ?>
